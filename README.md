...
2. Rebuild chunks: `python3 build_statewide_data.py` — reads **`dnr_wells_full.csv.gz`** from the repo directly (no `gzip`, no LFS required for the dataset). Optional wrappers: `./run_dnr_pipeline_local.sh` (simple) or `./run_dnr_pipeline_agentic.sh` (recommended for statewide HTML parsing: pilot + auto-retry + full run).
